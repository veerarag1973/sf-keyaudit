...
pub struct Cli {
    ...
    /// Each `.yaml` / `.yml` file in the directory must be a YAML list of
    /// `CustomRuleDef` entries.  Repeatable.
    ...
    /// Activate a named policy enforcement pack.
    ///
    /// Valid values: `strict-ci`, `developer-friendly` (default),
    /// `enterprise-default`, `regulated-env`, `custom`.
    ///
    /// Can also be set via the `policy.pack` field in the config file.
    /// CLI always takes precedence.
    #[arg(long, value_name = "PACK")]
    pub policy_pack: Option<String>,

    /// Append all security-relevant events to FILE in JSONL format.
    ///
    /// Each line is a self-contained JSON object with `timestamp`, `actor`,
    /// `scan_id`, and `event` fields.  The file is created if it does not
    /// exist; writes are `fsync`'d after each event.
    #[arg(long, value_name = "FILE")]
    pub audit_log: Option<PathBuf>,

    /// Identity recorded in the audit log for all events generated by this
    /// invocation.  Defaults to the value of `SFKEYAUDIT_ACTOR`, then
    /// `$USER` / `$USERNAME`, then `"unknown"`.
    #[arg(long, value_name = "IDENTITY")]
    pub actor: Option<String>,

    /// Repository identifier recorded in the audit log (e.g. `"org/repo"`).
    /// Defaults to `SFKEYAUDIT_REPOSITORY` env var, then empty string.
    #[arg(long, value_name = "REPO")]
    pub repository: Option<String>,

    /// Path to a triage state store file (`.sfkeyaudit-triage.json`).
    ///
    /// When supplied, the store is loaded at scan time and the recorded triage
    /// states are applied to matching findings before output is rendered.
    /// Use `sf-keyaudit triage set` to add entries to the store.
    #[arg(long, value_name = "FILE")]
...
pub enum SfSubcommand {
    ...
    /// Manage finding triage states.
    ///
    /// The triage store (`.sfkeyaudit-triage.json` by default) records a
    /// human-reviewed lifecycle decision for each finding fingerprint.  Use
    /// `--triage-store <FILE>` at scan time to apply the store to findings.
    Triage {
        #[command(subcommand)]
...
/// Sub-subcommands for `sf-keyaudit triage`.
#[derive(Debug, clap::Subcommand)]
pub enum TriageCommand {
    /// Set the triage state for a finding.
    ///
    /// Example:
    ///   sf-keyaudit triage set fp-a1b2c3d4 false_positive \\
    ///     --justification "test fixture, not a real key"
    Set {
        /// Fingerprint of the finding (e.g. `fp-a1b2c3d4e5f6g7h8`).
        fingerprint: String,

        /// New triage state.
        ///
        /// Valid values: `open`, `acknowledged`, `false_positive`,
        /// `accepted_risk`, `revoked_pending_removal`, `fixed`.
        state: String,

        /// Free-text justification (required for `accepted_risk`).
        #[arg(long)]
        justification: Option<String>,

        /// Path to the triage state store file.
        /// Defaults to `.sfkeyaudit-triage.json` in the current directory.
        #[arg(long, value_name = "FILE")]
        store: Option<PathBuf>,
    },

    /// List all entries in the triage store.
    List {
        /// Path to the triage state store file.
        /// Defaults to `.sfkeyaudit-triage.json` in the current directory.
        #[arg(long, value_name = "FILE")]
...
mod tests {
    ...
    #[test]
    fn policy_pack_flag() {
        let cli = parse(&["--policy-pack", "strict-ci"]);
        assert_eq!(cli.policy_pack, Some("strict-ci".to_string()));
    }

    #[test]
    fn audit_log_flag() {
        let cli = parse(&["--audit-log", "audit.jsonl"]);
        assert_eq!(cli.audit_log, Some(PathBuf::from("audit.jsonl")));
    }

    #[test]
    fn actor_flag() {
        let cli = parse(&["--actor", "ci-bot"]);
        assert_eq!(cli.actor, Some("ci-bot".to_string()));
    }

    #[test]
    fn repository_flag() {
        let cli = parse(&["--repository", "org/repo"]);
        assert_eq!(cli.repository, Some("org/repo".to_string()));
    }

    // ── Triage store flag ──────────────────────────────────────────────────────

    #[test]
    fn triage_store_flag() {
        let cli = parse(&["--triage-store", ".sfkeyaudit-triage.json"]);
        assert_eq!(cli.triage_store, Some(PathBuf::from(".sfkeyaudit-triage.json")));
    }

    #[test]
    fn triage_store_default_is_none() {
        let cli = parse(&[]);
        assert!(cli.triage_store.is_none());
    }

    // ── Triage subcommand ──────────────────────────────────────────────────────

    #[test]
    fn triage_set_subcommand_parses() {
        let cli = parse(&["triage", "set", "fp-abc123def", "false_positive"]);
        match cli.command {
            Some(SfSubcommand::Triage { command: TriageCommand::Set { ref fingerprint, ref state, .. } }) => {
                assert_eq!(fingerprint, "fp-abc123def");
                assert_eq!(state, "false_positive");
            }
            other => panic!("expected Triage::Set, got {other:?}"),
        }
    }

    #[test]
    fn triage_set_with_justification_parses() {
        let cli = parse(&["triage", "set", "fp-abc123def", "accepted_risk", "--justification", "low impact"]);
        match cli.command {
            Some(SfSubcommand::Triage { command: TriageCommand::Set { ref justification, .. } }) => {
                assert_eq!(justification.as_deref(), Some("low impact"));
            }
            other => panic!("expected Triage::Set with justification, got {other:?}"),
        }
    }

    #[test]
    fn triage_list_subcommand_parses() {
        let cli = parse(&["triage", "list"]);
        assert!(
            matches!(cli.command, Some(SfSubcommand::Triage { command: TriageCommand::List { .. } })),
            "expected Triage::List"
        );
    }
}
